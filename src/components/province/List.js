...
class List extends Component {
    ...
    loadData(){
      ...
      const url = `${API_BASE_URL}/provinces?size=${this.state.size}&page=${this.state.page}&sort=${this.state.sortProperty},${this.state.sortDirection}`;      
      fetch(url)
    ...
    async remove(id) {
      await fetch(`${API_BASE_URL}/province/${id}`, {
        method: 'DELETE',
        headers: {
    ...
    render() {
        ...
        const list = data.map(item => {            
            return <tr key={item.id}>
              ...
              <td style={{whiteSpace: 'nowrap'}}>{item.country.name}</td>
              <td>
                <ButtonGroup>
                  <Button size="sm" variant="primary" href={"/province/" + item.id}>Edit</Button>
            ...
            <div>              
              ...
              <Container fluid>
                <div className="float-right">
                  <Button variant="success" href="/province/new">Create new</Button>
                </div>
                <h3>Provinces</h3>
