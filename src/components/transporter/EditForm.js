...
class EditForm extends Component {
    ...
    async componentDidMount() { 
      ...
      if (this.props.match.params.id !== 'new') {
        const data = await (await fetch(`${API_BASE_URL}/transporter/${this.props.match.params.id}`)).json();
        Promise.all([
    ...
    async handleSubmit(event) {
      ...
      if (form.checkValidity() === false) {        
        ...
        await fetch(`${API_BASE_URL}/transporter`, {
    ...
    render() {
      ...
      const title = <h2>{item.id ? 'Edit Transporter' : 'Add Transporter'}</h2>;    
        ...
        <Container>
          ...
          <Form noValidate
            ...
            <Form.Group>
                ...
                <Button variant="secondary" href="/transporters">Cancel</Button>
            </Form.Group>        
          </Form>          
        </Container>
      </div>
